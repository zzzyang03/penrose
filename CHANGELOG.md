...
- HTTP 续播不再用 loadfile 的 `start=`（打开时尚未读到 Matroska Cues，HEVC / TrueHD 会停在 GOP 中间，进度条卡住直到再 seek）。改为先打开文件，再绝对 seek 到续播位置。打开音频直通后，如果输出设备本身不接收位流（例如笔记本扬声器），mpv 回退 PCM 后解码不再继续，声音和画面停在原处（从头播放同样如此，续播时表现为要拖一下进度条才继续）；现在会立即改用共享模式 PCM 继续播放，多声道也不会被独占模式压成立体声；横幅为「已回退为 PCM（7.1）」，控件栏在未下混时显示「7.1 PCM」，仅当下混时才出现箭头（「7.1 → 2.0 PCM」）。设置里的直通开关不变。切换 Windows HDR 后重新下发音频选项。
...
- HTTP resume no longer uses loadfile `start=` (that seeks before Matroska Cues
  ...
  The file opens, then an absolute seek goes to the resume position. With
  audio passthrough on, an output device that does not accept bitstream (such
  as laptop speakers) made mpv fall back to PCM and then stop decoding: sound
  and picture stayed put, from the start as well as on resume (where dragging
  the slider got it going). Playback now switches to shared-mode PCM right
  away, so it keeps playing and multichannel audio is not squeezed to stereo by
  exclusive mode. The fallback banner names the PCM layout (`7.1`); the
  channel chip shows `7.1 PCM`, and an arrow only when downmixing
  (`7.1 → 2.0 PCM`). The passthrough setting is left unchanged.
