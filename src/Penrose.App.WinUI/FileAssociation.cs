...
{
    private const string ProgId = "Penrose.MediaFile";
    ...
    private static string OpenCommand()
    {
        ...
        string exe = Path.Combine(dir, "Penrose.exe");
        // A self-contained publish carries its own runtime and can be the shell
        // verb directly. A framework-dependent dev build goes through the .cmd that
        // points DOTNET_ROOT at a user-local SDK when there is one (and flashes a console).
        bool selfContained = File.Exists(Path.Combine(dir, "coreclr.dll"))
            || File.Exists(Path.Combine(dir, "hostfxr.dll"));
        string launcher = Path.Combine(dir, "start-app-local.cmd");
