...
using MediaPlayer.VideoSurface;
using MediaPlayer.VideoSurface.WinUI;
using Microsoft.UI;
...
public sealed partial class MainWindow : Window
{
        ...
        try
        {
            ...
            _engine = new MpvPlaybackEngine(
                ...
                new EngineBootstrapOptions
                {
                    Osc = false,
                    OsdBar = false,
                    // MEDIAPLAYER_MPV_LOG=v|debug traces stream / demuxer activity into the app log.
                    MpvLogLevel = Environment.GetEnvironmentVariable("MEDIAPLAYER_MPV_LOG") is { Length: > 0 } level ? level : "warn",
                },
                surface,
            ...
            string? open = Environment.GetEnvironmentVariable("MEDIAPLAYER_DEBUG_OPEN");
            ...
            if (Environment.GetEnvironmentVariable("MEDIAPLAYER_DEBUG_BADGES") is { Length: > 0 } preview)
            {
            ...
            if (Environment.GetEnvironmentVariable("MEDIAPLAYER_DEBUG_PLAY_LIBRARY") is { Length: > 0 } pick)
            {
    ...
    private async Task<string> TestEmbyConnectionAsync(string urlText, string userName, string password)
    {
        ...
        try
        {
            using EmbyClient probe = new(url, Environment.MachineName, "mediaplayer-" + Environment.MachineName, AppVersion.Display);
...
#if DEBUG
            // MEDIAPLAYER_DEBUG_SEEK_TO_END_MINUS=<seconds>: land near the end so the
            // prefetch / auto-continue path can be exercised without watching it all.
            if (Environment.GetEnvironmentVariable("MEDIAPLAYER_DEBUG_SEEK_TO_END_MINUS") is { Length: > 0 } tail
