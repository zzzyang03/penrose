...
namespace MediaPlayer.VideoSurface.Abstractions.Tests;
...
public sealed class SurfaceLayoutTests
{
    ...
    {
        MediaPlayer.Core.Options.SurfaceBootstrapOptions empty = new();
        MediaPlayer.Core.Options.SurfaceBootstrapOptions scrgb = OutputPipeline.Apply(empty, OutputPipeline.ScRgb);
        Assert.Equal("rgba16f", scrgb.D3d11OutputFormat);
        Assert.Equal("linear", scrgb.D3d11OutputCsp);
        MediaPlayer.Core.Options.SurfaceBootstrapOptions pq = OutputPipeline.Apply(empty, OutputPipeline.Hdr10);
        Assert.Equal("rgb10_a2", pq.D3d11OutputFormat);
        Assert.Equal("pq", pq.D3d11OutputCsp);
        MediaPlayer.Core.Options.SurfaceBootstrapOptions sdr = OutputPipeline.Apply(empty, OutputPipeline.Sdr);
        ...
        DisplayColorCapabilities display = new(
            ...
            EffectiveGamut: "dci-p3",
            SdrWhiteLevel: 80);
        MediaPlayer.Core.Options.SurfaceBootstrapOptions injected = OutputPipeline.Apply(empty, OutputPipeline.ScRgb, display);
        Assert.Equal("1436", injected.TargetPeak);
        Assert.Equal("dci-p3", injected.TargetGamut);
        MediaPlayer.Core.Options.SurfaceBootstrapOptions sdrOnHdrDesktop = OutputPipeline.Apply(empty, OutputPipeline.Sdr, display);
